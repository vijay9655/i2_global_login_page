    ...
    if(data){
      ...
      axios.post("https://62172d3871e7672e5375b02f.mockapi.io/vijayapi/login",{
        ...
        if(res.status==201){
              ...
              localStorage.setItem("userlogin",JSON.stringify({name:data.email,password:data.password}))
  ...
  return (
    ...
    <div class="grid md:grid-cols-3 items-center shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-xl overflow-hidden">
      ...
      <form onSubmit={handleSubmit(onSubmit)} class="md:col-span-2 w-full py-6 px-6 sm:px-16">
        ...
        <div class="space-y-6">
          <div>
            ...
            <div class="relative flex items-center">
              <input 
              name="agentname" type="text" required class="text-gray-800 bg-white border border-gray-300 w-full text-sm px-4 py-2.5 rounded-md outline-blue-500" placeholder="Enter AgentName" 
            ...
            />
           {errors["agentname"]?.type === "required"||errors["agentname"]?.type === "pattern" && (
              <p style={{color:"red"}} className="text-red" role="alert">{errors["agentname"]?.message!=""?errors["agentname"]?.message:`agentname is required`}</p>
            )}
        ...
        <p class="text-gray-800 text-sm mt-6 text-center">Already Registered?<a href="/login" class="text-blue-600 font-semibold hover:underline ml-1">Login here</a></p>
      </form>
    </div>
  </div>
  )
